...
> **一键验收**：你只需准备两个 API key（见 **[USER_TODO.md](USER_TODO.md)** 手把手教程），把 **[ACCEPTANCE_PROMPT.md](ACCEPTANCE_PROMPT.md)** 整段喂给本地 Claude Code，其余（本地 LiveKit、起服务）全自动；然后开 `localhost:8080/voice` 对着麦克风说话、在 `localhost:8080/dashboard` 点"放行"。**无需注册 LiveKit/Discord 等任何账号。**
