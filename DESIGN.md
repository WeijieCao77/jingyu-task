...
抬杆控制器在**园区内网**、非公网，开发者笔记本无法直连。`notify/gate.py` 做了**双模式**：
- 默认 **stub**（打印"已发送抬杆指令"），demo 用；
- 配了 `HIKVISION_URL` 就走**真实 ISAPI**（digest auth 的 HTTP PUT），生产用——代码已就绪，进园区内网即生效：
```
PUT http://<controller-ip>/ISAPI/ITC/Entrance/barrierGateCtrl/channels/<ch>
<BarrierGate><cmd>open</cmd></BarrierGate>      # digest auth
```
答辩口径："不是只差一个调用——调用已经写好并单测，只差接入内网。"
