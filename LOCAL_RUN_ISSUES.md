...
## 2026-06-14 🔴 P0 回归：电话接通后"完全没声音"——livekit-agents 被动升 1.6.0 崩 job

**触发**：本轮把 Railway 部署源重指 main 并 push，触发**重新构建**；`requirements.txt` 当时是 `livekit-agents[...]~=1.5`（= <2.0），于是从实测可跑的 **1.5.17 升到了 1.6.0**。
...
**根因**：1.6.0 新增硬校验——RealtimeModel 用 server-side turn_detection 时，**不允许**再给 `AgentSession(allow_interruptions=False)`。而这正是本项目"防回声/防吞字"的默认组合（`build_realtime` 设 server_vad + `agent.py` 设 allow_interruptions=False）。1.5.17 允许、1.6.0 直接报错 → 每通电话 job 崩 → 主叫听到静音。

**修复（本轮，requirements.txt）**：整套 livekit 钉死到 **==1.5.17**（agents + openai/anthropic/silero/turn-detector 插件），精确复现实测可跑版本，不动防吞字逻辑。**注释里写明别再松回 `~=1.5`**。push → Railway 重建 → 电话恢复。

**教训/给远程**：① livekit 全家桶**必须钉精确版本**，`~=1.5` 会在重建时静默升 minor 引入破坏性变更；② 以后任何触发 rebuild 的操作（换分支/改 Dockerfile/手动 redeploy）后，**务必真打一通电话验证**，光看 `/health` 和 worker `registered` 不够——job 是接通才跑的。③ 若将来确要上 1.6.x：按报错提示改为"RealtimeModel 不设 turn_detection + 在 AgentSession 上挂 VAD/turn 检测"，并重测防吞字。
