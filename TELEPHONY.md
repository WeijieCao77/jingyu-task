...
**门卫用电话按键决定（DTMF）**：门卫接进来、和访客核对后，直接在手机上按键——
**按 1 = 放行**（系统开闸 + 记一条"已放行"，登记没填完也会从已采集信息建一条放行记录）；**按 2 = 拒绝**（记一条拒绝，不抬杆）。
接通时 AI 会念一句"按 1 放行、按 2 拒绝"提示门卫；Telegram 转人工提醒里也注明。无需再回后台点。
