...
| 35 | **访客拨打手机号和 agent 对话**（任务第一必交项） | `scripts/setup_sip.sh`(LiveKit 入站 trunk+dispatch)+`TELEPHONY.md`；Twilio→LiveKit SIP→自动派发 agent；电话汇入同一房间，全部能力复用 | ✅⏳ | `TELEPHONY.md` |
| 36 | 默认就用 realtime | `VOICE_MODE` 默认 realtime（真机 ≈1.4s），pipeline 一行回退 | ✅ | `config.py` |
| 37 | 黑名单登记不放行；白名单也仍需保安确认 | 两放行路径拒绝黑名单+Dashboard 禁放行；`AUTO_PASS_WHITELIST` 默认关（海康管已登记车牌，访客侧一律人工） | ✅ | `web/server.py` |
| 38 | 电话场景手机号 | SIP caller-ID(`sip.phoneNumber`)自动预填手机槽，省一问+免听错+触发匹配 | ✅⏳ | `agent.py` |
| 39 | 研究 QClaw/OpenClaw、realtime、选型（任务点名） | QClaw=腾讯个人微信路径；LiveKit SIP vs Vapi/Retell 取舍写进文档 | ✅ | `TELEPHONY.md`/`WECHAT_PLAN.md` |
...
**v0.23 电话接入(Twilio→LiveKit SIP，拨号进来) + 默认 realtime + 黑名单"登记不放行" + 主叫号预填手机**。
...
## 六、需要你拍板/检查的（部分已定，见 ✓）
- ✓ **默认语音架构**：已定 **realtime**（用户拍板）；pipeline 一行回退。
- ✓ **白名单自动放行**：已定**不自动放行**（`AUTO_PASS_WHITELIST` 默认关）——海康管已登记车牌，访客侧一律保安确认。
- ✓ **黑名单处置**：已定**登记不放行**（系统拒绝放行 + 告警）。
- ⏳ **电话真机验证**：代码/脚本/文档就绪，待你用 Twilio + LiveKit Cloud 拨打实测 + 25 秒计时（`TELEPHONY.md` §六 prompt）。
- 最终模型选型（客户）。
- 微信：demo=Telegram；生产=企业微信（方案 B 按钮回调）；个人微信=QClaw 可选——推进时机你定。
