...
### v0.17（分支 feature/data-matching）— 数据准确性 + 园区公司名单匹配
> 来源：真机测试发现车牌"粤A"记成"月月"、"蓝色鲸鱼"记成"蓝色金鱼"。
- **公司名单模糊匹配** `roster.py`：JSON 名单（支持别名）+ 汉字/拼音相似度匹配，把听错的单位纠正到官方名；接入 `RegistrationSession`，AI 确认纠正后的名称。**默认关闭**（`ROSTER_PATH` 配了才生效，不影响基础 demo）。
- **车牌省份确定性归一** `slots.normalize_plate`：省份名→简称（广东A→粤A 等 31 省）。
- 文档 `DATA_MATCHING_PLAN.md`（封闭集合用名单/开放集合用复述确认的整体策略）+ `roster.example.json` 示例；`pypinyin` 可选依赖。
- 测试 39→46。
- **计划变更**：数据准确性策略定型——单位用名单匹配、车牌省份确定性映射、号码/车牌号靠主分支的 AI 复述确认。
