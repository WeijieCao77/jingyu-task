...
- **操作层（真机电话反馈）**：
  - FR-3 温和开场白（不再冷冰冰三连问）；FR-6 字母带消歧词回读（B,Boy 的 B）+ 不确定主动反问；
  - FR-7 更正后只复述被改项、不重念已确认项；FR-8 手机 11 位校验、位数不对请重报；
  - FR-5 单位不在名单 → 把信号给 LLM（先确认再转人工）+ 扩展转人工触发 + **转人工同时推门卫手机**（`dispatch.push_alert`）；
  - FR-9 无具体公司（参观/物业）不强逼、不编造（新增"绝不编造"硬规则）；
  - FR-4 登记完静默自动挂断（`HANGUP_SILENCE_SEC`）+ 单通最长时长兜底（`MAX_CALL_SEC`）；
  - sim 注入 `access_check`（黑白名单可离线测）+ 用真 `GREETING`。
- **模型架构层（效率/准确/省钱）**：数据查询**强制先调工具**（杜绝 gpt-4o-mini "凭空答 0/拒答"）+ system 严禁臆造；
  **模型分层** `GUARD_QUERY_MODEL`（查询可用与大脑不同档位）。`UPGRADE_PLAN.md §二` 列出 token/成本/准确的后续杠杆。
- **产品化层（多租户起步）**：`tenant.py` 按被叫号码路由到各租户独立配置（roster/access/notify/门卫/URL），
  `TENANTS_PATH` 开关默认关、单租户行为不变；`tenants.example.json` + 单测 + agent opt-in 接入。`UPGRADE_PLAN.md §三` 给出阶段 2~5（数据隔离/自助开通/控制台/计费）。
- **测试健壮性**：`tests/conftest.py` 让全套测试忽略磁盘 `.env`（一次性修好 P2-4 + 开 `GUARD_ACCESS_KEY` 后的鉴权用例隔离）。
- **新增文档**：`UPGRADE_PLAN.md`（三层升级计划）。
- **计划变更**：确立"三层升级"路线（操作/模型架构/产品化），多租户作为产品化第一块地基落地；其余按 `UPGRADE_PLAN.md` 分阶段推进。

## 2026-06-12

### v0.25 — 数据中心(对话+筛选) + SQLite 持久化修复 + 门卫专属访问(口令/手机白名单)
> 用户连发：查询要"对话 + 机械筛选都全面"；每次测试数据被清空、回访识别不到；查询要**只有门卫能用**（口令 或 1~2 个门卫手机号区分）。
- **改动**：
  - **数据中心 `/ask` 双模**：💬对话（LLM 多轮）+ 🔎**筛选查询**（确定性、不走 LLM：今天/本周/本月/全部 + 单位/车牌/状态 → 精确数量 + 已放行数 + 时段直方图 + 结果表）。新增 `GET /api/query` + `repo.query_visits`（参数化只读）。
  - **SQLite 持久化修复**：`DATABASE_URL=sqlite:///./data/visits.db` 是**相对当前目录**的——用户在 Windows 不同目录启动 python，导致每次/每进程命中不同的空库（看起来"被清空"、回访识别不到）。`_resolve_sqlite_url()` 把相对路径**锚定到项目根**，agent+web+每次运行共用同一文件；启动日志打印绝对路径。绝对路径/Postgres 原样放行。
  - **门卫专属访问（访客不能查）**：
    · 网页 `GUARD_ACCESS_KEY`：中间件守 `/dashboard /ask /admin` + 数据 API；`/login` 输口令（cookie/Header/`?key=` 皆可）；访客页 `/voice /qr` 与带 token 的 `/confirm` 仍公开。留空=不设防。
    · 电话 `GUARD_PHONES`：来电号码在门卫白名单 → 路由到 **`GuardQueryAgent` 语音数据助手**（同一套安全只读工具，语音问答），否则按访客登记。默认不设=零行为变化。
- **计划变更**：查询从单一形态升级为**对话+结构化双模**且**带鉴权/角色区分**（门卫 vs 访客）；DB 默认更"稳"（锚定根目录）。测试 78→81。

### v0.24 — 门卫数据助手：对话式查询页 `/ask`（多轮追问）+ 放行维度
> 用户要求：不是自己在 dashboard 一条条翻，而是**像和 AI 助手对话**那样问数据。
- **改动**：
  - **`/ask` 对话式查询页**：聊天气泡 UI、示例问题 chips、「新对话」、回车发送；前端维护会话、每次带 `history` → **支持追问**（"那上个月呢？""他呢？"）。从 `/dashboard` 头部「🔎 数据查询」入口进。
  - **多轮上下文**：`guard_query.answer_question(question, history=...)` + `_clean_history()`；`/guard/query` 接受 `history`；system prompt 提示结合上文理解省略问法。
  - **放行维度**：`count_visits` 加 `status` 过滤（confirmed=已放行/pending=待核对）→ 能答"这个月**多少辆车被放行**"。"找哪家公司多少人""什么时段最多""某人来几次"原有工具已覆盖。
  - 复用既有安全设计：LLM 只能调参数化只读工具（count_visits/list_visits/busiest_hours/frequent_visitors），非 text-to-SQL，无注入面。
- **计划变更**：门卫查询从"API + CLI"升级为**面向保安的对话式网页助手**（任务加分项"门卫查询 Agent"做成可演示形态）；语音电话查询（打电话问数据）复用同一 `answer_question`，作为后续。测试 75→78。
