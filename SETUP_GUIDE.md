...
2. 字母消歧：报带字母车牌（如沪AB123D）→ AI 复述是否说"B，Boy 的 B""D，Dog 的 D"；听不清是否主动反问。
3. 只复述被改项：报全 4 项让它复述，然后只改手机号 → AI 是否只复述新手机、不再重念车牌。
4. 手机校验：故意报 10 位号码 → AI 是否指出位数不对、请重报。
5. 单位不在名单：报一个 roster.example.json 里没有的公司 → AI 是否先确认一次、仍对不上则转人工并说"已通知门卫"。
6. 转人工推送：触发转人工后，我的 Telegram 是否收到"⚠️访客请求转人工 + 房间 + 介入链接"。
7. 无具体公司：说"我来参观园区" → AI 是否不强求公司、也不编造。
8. 自动挂断：登记完道别后我保持沉默 ~6 秒 → 通话是否自动挂断（agent 日志有 "hang up"）。

【数据查询】
9. 对话模式 /ask：问"这个月放行了多少辆车" → 是否先调工具再答、数字对（不再答 0/拒答）；追问"那本周呢"。
10.（可选）电话门卫查询：.env 设 GUARD_PHONES=我的手机号，用该号拨入 → AI 是否变"数据助手"；换号拨则正常登记。

【多租户（可选预览）】
11.（想试再做）复制 tenants.example.json 为 tenants.json 改成你两个号码、设 TENANTS_PATH=tenants.json →
    不同号码拨入是否用各自名单/通知/门卫（agent 日志 "tenant=…"）。
